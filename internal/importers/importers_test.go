...
func openImporterTestDB(t *testing.T) (*database.DB, apppaths.Paths) {
	t.Helper()
...
func TestPreviewAndMaterializeSupportedTextFormats(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.file, func(t *testing.T) {
			...
			source, err := service.Materialize(ctx, MaterializeRequest{Path: fixture(test.file)})
			if err != nil {
				t.Fatal(err)
			}
			if source.RowCount != int64(test.wantRows) || source.Schema != "data" || len(source.Columns) == 0 {
				t.Fatalf("unexpected source: %#v", source)
			}
		})
	}
	state, err := workspace.New(db).Bootstrap(ctx)
	if err != nil {
		t.Fatal(err)
	}
...
func TestValidationErrorsAndStagingCleanup(t *testing.T) {
	...
	if _, err := service.Materialize(context.Background(), MaterializeRequest{Path: broken}); err == nil {
...
func TestWorkbookSheetListingAndConditionalImport(t *testing.T) {
	...
	source, err := service.Materialize(ctx, MaterializeRequest{Path: workbookPath, Sheet: "Sheet1"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestMetadataPersistsAcrossReopen(t *testing.T) {
	...
	service := New(db)
	source, err := service.Materialize(context.Background(), MaterializeRequest{Path: fixture("people.csv")})
	if err != nil {
		t.Fatal(err)
	}
	if _, err := workspace.New(db).CreateSavedQuery(context.Background(), "Adults", `SELECT * FROM data.people WHERE age >= 18`); err != nil {
		t.Fatal(err)
	}
